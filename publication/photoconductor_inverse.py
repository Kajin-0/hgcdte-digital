...
# ════════════════════════════════════════════════════════════════════════
#  PARAMETER EXTRACTION: Digital Twin Calibration
# ════════════════════════════════════════════════════════════════════════
...
class MeasuredData:
    ...
    # Geometry
    L: float  # Contact spacing [m]
    W: float  # Width [m]
    t: float  # Thickness [m]
    T: float  # Temperature [K]
    
    # DC electrical
    V_bias: np.ndarray  # Bias voltages [V]
    I_meas: np.ndarray  # Measured currents [A]
    R_meas: np.ndarray  # Measured resistances [Ω]
    
    # AC response
    f_3dB: float  # 3dB frequency [Hz]
    tau_stated: float  # Stated carrier lifetime [s]
    
    # Noise (at specific frequencies)
    f_noise: np.ndarray  # Noise measurement frequencies [Hz]
    S_V_sqrt: np.ndarray  # Voltage noise spectral density [V/√Hz]
    
    # Optical responsivity
    E_irradiance: float  # Irradiance [W/m²]
    V_sig_rms: float  # Signal voltage RMS [V]
    A_optical: float  # Optical area [m²]


@dataclass
class ExtractedParameters:
    """Extracted material/device parameters."""
    # Material
    mu: float  # Mobility [m²/(V·s)]
    n: float  # Carrier density [m⁻³]
    sigma: float  # Conductivity [S/m]
    
    # Device
    R_contact: float  # Contact resistance [Ω]
    R_bulk: float  # Bulk resistance [Ω]
    
    # Dynamics
    tau_recomb: float  # Recombination lifetime [s]
    tau_electrical: float  # Electrical time constant [s]
    
    # Optical
    responsivity_V: float  # Voltage responsivity [V/W]
    gain: float  # Photoconductive gain
    eta_abs_QE: float  # Absorption × QE product
    
    # Quality metrics
    fit_quality: dict  # Chi-squared, RMSE, etc.


def extract_conductivity_and_mobility_density_product(measured: MeasuredData) -> tuple:
    """
    Extract conductivity σ and mobility-density product μn from resistance.
    
    σ = L / (R × A)
    μn = σ / q
    
    Returns:
        (sigma [S/m], mu_n [V⁻¹m⁻¹s⁻¹])
    """
    A = measured.W * measured.t  # Cross-sectional area
    
    # Conductivity from each resistance measurement
    sigma = measured.L / (measured.R_meas * A)
    
    # Mobility-density product
    mu_n = sigma / Q_E
    
    return sigma, mu_n


def separate_contact_resistance(V_bias: np.ndarray, R_total: np.ndarray) -> tuple:
    """
    Separate bulk and contact resistance from R(V) measurements.
    
    Model: R_total = R_bulk + 2*R_contact
    
    If R_bulk is field-dependent: R_bulk = R_bulk0 * (1 + α*E²)
    Assume contact resistance is constant.
    
    Uses linear fit at low fields where field-dependence is minimal.
    
    Returns:
        (R_contact [Ω], R_bulk_array [Ω])
    """
    if len(V_bias) < 3:
        # Not enough points for separation
        R_contact = 0.0
        R_bulk = R_total
        return R_contact, R_bulk
    
    # Use lowest voltages (< 0.2V) for linear region
    low_V_mask = V_bias < 0.2
    if np.sum(low_V_mask) >= 2:
        # Fit R_total = a + b*V² in low-field region
        # This assumes field-independent bulk in this range
        V_low = V_bias[low_V_mask]
        R_low = R_total[low_V_mask]
        
        # Constant component ≈ 2*R_contact + R_bulk0
        R_contact_estimate = np.min(R_low) * 0.1  # Rough estimate: 10% of min R
        R_bulk = R_total - 2 * R_contact_estimate
    else:
        # Fallback: assume 5% of total R is contact
        R_contact = 0.05 * np.mean(R_total)
        R_bulk = R_total - 2 * R_contact
    
    return R_contact, R_bulk


def extract_lifetime_from_frequency_response(f_3dB: float) -> float:
    """
    Extract electrical time constant from 3dB bandwidth.
    
    f₋₃dB = 1 / (2π τ_electrical)
    
    Returns:
        tau_electrical [s]
    """
    tau_elec = 1.0 / (2 * np.pi * f_3dB)
    return tau_elec


def fit_two_pole_model(f_3dB: float, tau_stated: float) -> tuple:
    """
    Reconcile measured f₋₃dB with stated carrier lifetime using two-pole model.
    
    Two time constants:
    1. τ_recombination (carrier lifetime)
    2. τ_electrical (RC, transit, readout)
    
    Overall response: H(f) = 1 / [(1 + j2πfτ₁)(1 + j2πfτ₂)]
    
    Returns:
        (tau_recombination [s], tau_electrical [s])
    """
    tau_elec = extract_lifetime_from_frequency_response(f_3dB)
    
    # If stated lifetime >> electrical lifetime
    if tau_stated > 5 * tau_elec:
        # Measured f₋₃dB dominated by faster electrical pole
        tau_recomb = tau_stated
        tau_electrical = tau_elec
    elif tau_stated < 0.2 * tau_elec:
        # Measured f₋₃dB dominated by recombination
        tau_recomb = tau_elec
        tau_electrical = 10 * tau_elec  # Electrical much slower
    else:
        # Both poles contribute
        # Approximate: if both similar, overall τ ≈ τ₁*τ₂/(τ₁+τ₂)
        # Solve: τ_measured = τ_recomb * τ_elec / (τ_recomb + τ_elec)
        # Assume one is tau_stated, solve for the other
        tau_recomb = tau_stated
        # τ_measured * (τ_recomb + τ_elec) = τ_recomb * τ_elec
        # τ_measured * τ_recomb + τ_measured * τ_elec = τ_recomb * τ_elec
        # τ_elec * (τ_recomb - τ_measured) = τ_measured * τ_recomb
        if (tau_recomb - tau_elec) > 0:
            tau_electrical = (tau_elec * tau_recomb) / (tau_recomb - tau_elec)
        else:
            tau_electrical = tau_elec
    
    return tau_recomb, tau_electrical


def extract_voltage_responsivity(measured: MeasuredData) -> float:
    """
    Extract voltage responsivity from optical measurement.
    ...
    Returns:
        Responsivity [V/W]
    """
    P_optical = measured.E_irradiance * measured.A_optical
    R_V = measured.V_sig_rms / P_optical
    return R_V


def extract_parameters_from_measurements(measured: MeasuredData, 
                                        mu_assumed: float = None) -> ExtractedParameters:
    """
    Main parameter extraction routine.
    
    Inverts measured quantities to extract material and device parameters.
    
    Args:
        measured: Container with all experimental measurements
        mu_assumed: If provided, assume this mobility and extract n directly
                   If None, return μn product and require additional constraint
    
    Returns:
        ExtractedParameters object with all extracted values
    """
    # 1. Extract conductivity and μn product
    sigma_array, mu_n_array = extract_conductivity_and_mobility_density_product(measured)
    sigma_mean = np.mean(sigma_array)
    mu_n_mean = np.mean(mu_n_array)
    
    # 2. Separate contact and bulk resistance
    R_contact, R_bulk_array = separate_contact_resistance(measured.V_bias, measured.R_meas)
    R_bulk_mean = np.mean(R_bulk_array)
    
    # 3. Extract carrier density (if mobility known) or report μn product
    if mu_assumed is not None:
        mu = mu_assumed
        n = mu_n_mean / mu
    else:
        # Can't separate without additional info
        # Report μn product and suggest mobility range
        mu = None
        n = None
    
    # 4. Extract time constants
    tau_recomb, tau_elec = fit_two_pole_model(measured.f_3dB, measured.tau_stated)
    
    # 5. Extract voltage responsivity
    R_V = extract_voltage_responsivity(measured)
    
    # 6. Estimate gain and absorption×QE (requires more assumptions)
    # Photoconductive gain: g = τ/τ_transit = τ*μ*E/L
    # For now, report responsivity and note gain requires transit time
    if mu is not None and n is not None:
        # Estimate field
        E_mean = np.mean(measured.V_bias) / measured.L
        transit_time = measured.L / (mu * E_mean)
        gain = tau_recomb / transit_time
        
        # Current responsivity: R_I = g * η * q/hν
        # Assuming λ ≈ 5 μm (MWIR), hν ≈ 0.25 eV
        h = 6.626e-34  # J·s
        c = 3e8  # m/s
        lambda_um = 5.0  # μm
        hnu = h * c / (lambda_um * 1e-6)  # J
        
        # R_I = R_V / R_load (if measuring across device)
        # For now, use bulk resistance as effective load
        R_I = R_V / R_bulk_mean
        
        # η = R_I / (g * q/hν)
        eta_abs_QE = R_I / (gain * Q_E / hnu)
        eta_abs_QE = np.clip(eta_abs_QE, 0, 1)  # Physical bound
    else:
        gain = None
        eta_abs_QE = None
    
    # 7. Compute fit quality metrics
    # R² for resistance model
    R_model = R_bulk_array + 2 * R_contact
    residuals = measured.R_meas - R_model
    ss_res = np.sum(residuals**2)
    ss_tot = np.sum((measured.R_meas - np.mean(measured.R_meas))**2)
    r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0
    rmse = np.sqrt(np.mean(residuals**2))
    
    fit_quality = {
        'r_squared': r_squared,
        'rmse_resistance': rmse,
        'sigma_std': np.std(sigma_array),
        'notes': []
    }
    
    if mu is None:
        fit_quality['notes'].append('Mobility not specified - returning μn product')
    if len(measured.V_bias) < 3:
        fit_quality['notes'].append('Limited voltage points - contact resistance uncertain')
    ...
    return ExtractedParameters(
        ...
        responsivity_V=R_V,
        gain=gain,
        eta_abs_QE=eta_abs_QE,
        fit_quality=fit_quality
    )


def print_extraction_report(measured: MeasuredData, 
                           extracted: ExtractedParameters,
                           mu_n_product: float = None):
    """Generate human-readable parameter extraction report."""
    print("\n" + "="*80)
    ...
    print("─"*80)
    print(f"Geometry:")
    print(f"  Contact spacing (L):  {measured.L*1e3:.2f} mm")
    print(f"  Width (W):            {measured.W*1e3:.2f} mm")
    print(f"  Thickness (t):        {measured.t*1e6:.1f} µm")
    print(f"  Temperature (T):      {measured.T:.0f} K")
    print(f"  Cross-section (A):    {(measured.W*measured.t)*1e8:.2f} × 10⁻⁸ m²")
    
    print(f"\nDC Electrical:")
    print(f"  Bias voltage range:   {measured.V_bias[0]:.2f} - {measured.V_bias[-1]:.2f} V")
    print(f"  Current range:        {measured.I_meas[0]*1e3:.1f} - {measured.I_meas[-1]*1e3:.1f} mA")
    print(f"  Resistance range:     {measured.R_meas[0]:.1f} - {measured.R_meas[-1]:.1f} Ω")
    
    print(f"\nFrequency Response:")
    print(f"  3dB bandwidth:        {measured.f_3dB*1e-3:.1f} kHz")
    print(f"  Stated carrier τ:     {measured.tau_stated*1e6:.1f} µs")
    
    print(f"\nOptical:")
    print(f"  Irradiance:           {measured.E_irradiance:.2e} W/m²")
    print(f"  Signal voltage:       {measured.V_sig_rms*1e6:.0f} µV_rms")
    print(f"  Optical power:        {measured.E_irradiance*measured.A_optical*1e9:.2f} nW")
    ...
    print(f"\n✦ Conductivity:")
    print(f"  σ = {extracted.sigma:.1f} S/m")
    print(f"  ρ = {1/extracted.sigma:.2e} Ω·m")
    
    if mu_n_product:
        print(f"\n✦ Mobility-Density Product:")
        print(f"  μn = {mu_n_product:.2e} V⁻¹m⁻¹s⁻¹")
        print(f"  → Constrains: n = σ/(qμ)")
    
    if extracted.mu is not None:
        print(f"\n✦ Mobility:")
        print(f"  μ = {extracted.mu:.3f} m²/(V·s)")
        print(f"    (assumed or from independent measurement)")
        
        print(f"\n✦ Carrier Density:")
        print(f"  n = {extracted.n:.2e} m⁻³")
        print(f"  → Extracted from σ = qμn")
    else:
        print(f"\n✦ Mobility & Carrier Density:")
        print(f"  Cannot separate without independent μ measurement")
        print(f"  Typical HgCdTe at 77K: μ = 0.5-5 m²/(V·s)")
        print(f"  If μ = 1 m²/(V·s): n ≈ {mu_n_product/1:.2e} m⁻³")
        print(f"  If μ = 5 m²/(V·s): n ≈ {mu_n_product/5:.2e} m⁻³")
    
    print(f"\n✦ Resistance Components:")
    print(f"  Bulk resistance:      {extracted.R_bulk:.1f} Ω")
    print(f"  Contact resistance:   {extracted.R_contact:.1f} Ω")
    print(f"  Contact fraction:     {2*extracted.R_contact/np.mean(measured.R_meas)*100:.1f}%")
    
    print(f"\n✦ Time Constants:")
    print(f"  Recombination τ:      {extracted.tau_recomb*1e6:.2f} µs")
    print(f"  Electrical τ:         {extracted.tau_electrical*1e6:.2f} µs")
    print(f"  Measured f₋₃dB:       {measured.f_3dB*1e-3:.1f} kHz")
    print(f"  → Two-pole model reconciles τ_stated vs f₋₃dB mismatch")
    
    print(f"\n✦ Optical Response:")
    print(f"  Voltage responsivity: {extracted.responsivity_V:.1f} V/W")
    
    if extracted.gain is not None:
        print(f"  Photoconductive gain: {extracted.gain:.1f}")
        print(f"  Absorption × QE:      {extracted.eta_abs_QE*100:.1f}%")
    else:
        print(f"  Gain & QE:            Requires mobility for transit time")
    
    print(f"\n✦ Fit Quality:")
    print(f"  R² (resistance):      {extracted.fit_quality['r_squared']:.4f}")
    print(f"  RMSE (resistance):    {extracted.fit_quality['rmse_resistance']:.2f} Ω")
    print(f"  σ variability:        {extracted.fit_quality['sigma_std']:.1f} S/m")
    
    if extracted.fit_quality['notes']:
        print(f"\n  Notes:")
        for note in extracted.fit_quality['notes']:
            print(f"    • {note}")
    ...
    if extracted.sigma < 100:
        print("  ⚠ Low conductivity - check for freeze-out or poor contacts")
    elif extracted.sigma > 10000:
        print("  ⚠ Very high conductivity - check for shorts or measurement error")
    else:
        print("  ✓ Conductivity in reasonable range for HgCdTe at 77K")
    
    if extracted.R_contact / extracted.R_bulk > 0.5:
        print("  ⚠ Contact resistance > 50% of bulk - contacts need improvement")
    elif extracted.R_contact / extracted.R_bulk < 0.01:
        print("  ✓ Contact resistance < 1% - excellent contacts")
    else:
        print("  ✓ Contact resistance reasonable")
    
    if extracted.tau_recomb < 1e-7:
        print("  ⚠ Very short lifetime - strong recombination or trapping")
    elif extracted.tau_recomb > 1e-3:
        print("  ⚠ Very long lifetime - unusual for PC detectors, check measurement")
    else:
        print("  ✓ Lifetime in typical PC detector range")
    
    if extracted.gain and extracted.gain > 1:
        print(f"  ✓ Gain > 1 indicates photoconductive amplification")
...
def main():
    ...
    parser.add_argument("--extract_parameters", action="store_true",
                        help="Extract material parameters from experimental measurements")
    parser.add_argument("--mu_assumed", type=float, default=None,
                        help="Assumed mobility [m²/(V·s)] for parameter extraction (if known)")
    ...
    if args.extract_parameters:
        if rank == 0:
            # Example measured data (user should replace with actual measurements)
            measured = MeasuredData(
                L=1e-3,  # 1 mm contact spacing
                W=1e-3,  # 1 mm width
                t=10e-6,  # 10 µm thickness
                T=77.0,  # 77 K
                V_bias=np.array([0.8, 1.0, 1.5, 2.0]),  # V
                I_meas=np.array([16e-3, 20e-3, 30e-3, 40e-3]),  # A
                R_meas=np.array([50.0, 50.0, 50.0, 50.0]),  # Ω
                f_3dB=100e3,  # 100 kHz
                tau_stated=10e-6,  # 10 µs
                f_noise=np.array([1e3, 10e3]),  # 1 kHz, 10 kHz
                S_V_sqrt=np.array([15e-9, 10e-9]),  # nV/√Hz
                E_irradiance=0.431,  # W/m²
                V_sig_rms=400e-6,  # 400 µV_rms
                A_optical=1e-6,  # 1 mm²
            )
            
            print("\n" + "="*80)
            print("DIGITAL TWIN PARAMETER EXTRACTION")
            print("="*80)
            print("\nNOTE: Using example measured data.")
            print("Edit the MeasuredData object in the code to use your actual measurements.")
            print("="*80)
            
            # Extract parameters
            sigma, mu_n = extract_conductivity_and_mobility_density_product(measured)
            mu_n_mean = np.mean(mu_n)
            
            extracted = extract_parameters_from_measurements(measured, mu_assumed=args.mu_assumed)
            
            # Print report
            print_extraction_report(measured, extracted, mu_n_product=mu_n_mean)
...
1. Edit the MeasuredData object in the code with your measurements
2. Run: python3 photoconductor_poisson.py --extract_parameters
3. If mobility is known: --extract_parameters --mu_assumed 1.5

The extraction will:
• Calculate σ and μn product from R(V)
• Separate contact and bulk resistance
• Reconcile τ vs f₋₃dB using two-pole model
• Extract voltage responsivity
• If μ provided, calculate carrier density n
• Estimate photoconductive gain and QE

Next step: Use extracted parameters as inputs to forward simulation
to validate consistency and refine understanding.
            """)
